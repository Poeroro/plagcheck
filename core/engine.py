...
# Default thresholds (can be overridden in __init__)
DEFAULT_NEAR = 0.30
...
def _windowed_jaccard(query_shingles: set[str], corpus_text: str,
    ...
    """Slide a window over corpus_text, return (max_jaccard, best_window_text)."""
...
class PlagEngine:
    """High-level orchestration for a plagiarism check."""
    ...
    def __init__(self, corpus: Corpus, *, near_threshold: float = DEFAULT_NEAR,
        ...
        # Embedding cache (lazy built on first semantic check)
    ...
    def _get_corpus_embeddings(self, corpus_chunks: list[CorpusDoc]):
        """Return cached embeddings for the current corpus, building if needed."""
        import numpy as np

        # Identity of "current corpus" = list of doc_ids
        current_ids = [c.doc_id for c in corpus_chunks]
        cache_file = self.cache_dir / "corpus_emb.npz"
        meta_file = self.cache_dir / "corpus_ids.json"

        # Try load from disk
        if cache_file.exists() and meta_file.exists():
            try:
                import json
                saved_ids = json.loads(meta_file.read_text(encoding="utf-8"))
                if saved_ids == current_ids and len(saved_ids) == len(corpus_chunks):
                    data = np.load(cache_file)
                    self._cached_chunks = corpus_chunks
                    self._cached_embeddings = data["emb"]
        ...
        emb = self._bi_encoder.encode(texts, normalize_embeddings=True,
        ...
        try:
            ...
            import json
            meta_file.write_text(json.dumps(current_ids, ensure_ascii=False),
                                 encoding="utf-8")
        except Exception as e:  # noqa: BLE001
            print(f"[engine] cache save failed: {e}")
    ...
    def invalidate_cache(self) -> None:
        """Force rebuild on next semantic check (e.g. after corpus update)."""
    ...
    def check(self, file_path: str | Path, top_k: int = 5,
              ...
              min_paragraph_chars: int = 80) -> CheckResult:
        """Run a check.
        use_semantic: enable paraphrase detection (Phase 2).
        use_cross_encoder: rerank borderline semantic matches for higher precision.
        strip_citations: drop quoted/cited/bibliography text before matching.
        """
        ...
        # Pre-compute corpus embeddings if semantic enabled
        corpus_emb = None
        if use_semantic and self._bi_encoder is not None:
            corpus_emb = self._get_corpus_embeddings(corpus_chunks)
        ...
        for para in paragraphs:
            ...
            sem_hits: list[tuple[float, CorpusDoc]] = []
            if use_semantic and corpus_emb is not None and not near_hits:
                import numpy as np
                para_emb = self._bi_encoder.encode([para], normalize_embeddings=True,
                                                   show_progress_bar=False)
                sims = (corpus_emb @ para_emb.T).flatten()
                order = np.argsort(-sims)[:3]
                for o in order:
                    s = float(sims[o])
                    if s >= self.semantic_threshold:
                        sem_hits.append((s, corpus_chunks[o]))
